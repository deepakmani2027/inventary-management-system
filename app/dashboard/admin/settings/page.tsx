  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        ...
        const fallbackFullName = user.user_metadata?.full_name || 'Admin User'
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/users/${user.id}`, {
        ...
        body: JSON.stringify({ full_name: form.full_name, email: form.email, role: profile?.id ? undefined : 'admin' }),
      })
      ...
      <section className="relative overflow-hidden rounded-3xl border border-border/70 bg-linear-to-br from-background via-card/70 to-background p-6 shadow-2xl shadow-slate-950/10 backdrop-blur sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl space-y-4">
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
                Manage profile details and security controls from a dedicated admin workspace.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 shadow-sm backdrop-blur">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">{profile?.full_name || 'Admin account'}</p>
              <p className="text-sm text-muted-foreground">{profile?.email || 'Loading account details...'}</p>
              <p className="mt-1 text-xs uppercase tracking-[0.2em] text-muted-foreground">Admin only access</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
        <CardContent className="grid gap-4 pt-6 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
            ...
            <p className="mt-1 font-semibold">Admin only</p>
          </div>
        </CardContent>
      </Card>

      <Tabs value={activeTab} onValueChange={value => setActiveTab(value as 'profile' | 'security')} className="space-y-6">
        <TabsList className="grid h-14 w-full max-w-4xl grid-cols-2 rounded-full border border-border/70 bg-background/90 p-1 shadow-sm shadow-slate-950/5 backdrop-blur">
          <TabsTrigger value="profile" className="flex h-12 items-center justify-center rounded-full px-4 text-sm font-medium text-muted-foreground transition-all hover:text-foreground data-[state=active]:bg-slate-950 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-slate-950/20">
            Profile
          </TabsTrigger>
          <TabsTrigger value="security" className="flex h-12 items-center justify-center rounded-full px-4 text-sm font-medium text-muted-foreground transition-all hover:text-foreground data-[state=active]:bg-slate-950 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-slate-950/20">
            Security
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
            <Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>Quick view of your admin identity.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4 rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-base font-semibold">{profile?.full_name || 'Admin account'}</p>
                ...
                <div className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex items-center gap-2 rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
                    ...
                    <span>Display name for admin workspace and logs</span>
                  </div>
                  <div className="flex items-center gap-2 rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
            ...
            <Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
              ...
              <CardContent className="grid gap-4">
                ...
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm text-muted-foreground">
                  <span>Profile changes are saved for the current admin account.</span>
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                </div>
                <Button className="bg-linear-to-r from-slate-950 to-slate-700 text-white" onClick={updateProfile} disabled={loading || profileLoading}>{loading ? 'Saving...' : 'Save Profile'}</Button>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
          ...
          <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
            ...
            <Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
              ...
              <CardContent className="grid gap-4">
                ...
                <Button className="bg-linear-to-r from-slate-950 to-slate-700 text-white" onClick={changePassword} disabled={loading}>{loading ? 'Updating...' : 'Change Password'}</Button>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
