...
export default function InventoryManagerSettingsPage() {
	...
	useEffect(() => {
		const load = async () => {
			...
			if (data) {
				...
				const fallbackFullName = user.user_metadata?.full_name || 'Inventory Manager'
	...
	const updateProfile = async () => {
		...
		try {
			...
			const response = await fetch(`/api/admin/users/${user.id}`, {
				...
				body: JSON.stringify({ full_name: form.full_name, email: form.email, role: 'inventory_manager' }),
			})
				...
				<div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
					<div className="max-w-3xl space-y-4">
						<div className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/80 px-4 py-2 text-sm backdrop-blur">
							...
							Inventory manager account controls
						</div>
						<div>
							<h1 className="text-3xl font-semibold tracking-tight text-balance sm:text-5xl">Settings</h1>
							<p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
								Manage profile details and security controls from a dedicated inventory workspace.
							</p>
						</div>
					</div>
					<div className="flex items-center gap-4 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 shadow-sm backdrop-blur">
						...
						<div>
							<p className="text-sm font-medium text-foreground">{profile?.full_name || 'Inventory manager account'}</p>
							<p className="text-sm text-muted-foreground">{profile?.email || 'Loading account details...'}</p>
							<p className="mt-1 text-xs uppercase tracking-[0.2em] text-muted-foreground">Inventory manager access</p>
						</div>
					</div>
				</div>
			</section>
			...
			<Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
				<CardContent className="grid gap-4 pt-6 md:grid-cols-3">
					...
					<div className="rounded-2xl border border-border/70 bg-background/80 p-4">
						...
						<p className="mt-1 font-semibold">Inventory manager only</p>
					</div>
				</CardContent>
			</Card>
					...
					<div className="grid gap-6 lg:grid-cols-[320px_1fr]">
						<Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
							<CardHeader>
								...
								<CardDescription>Quick view of your inventory identity.</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div className="flex items-center gap-4 rounded-2xl border border-border/70 bg-background/80 p-4">
									...
									<div className="min-w-0">
										<p className="truncate text-base font-semibold">{profile?.full_name || 'Inventory manager account'}</p>
								...
								<div className="space-y-3 text-sm text-muted-foreground">
									<div className="flex items-center gap-2 rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
										...
										<span>Display name for inventory workspace and logs</span>
									</div>
									<div className="flex items-center gap-2 rounded-2xl border border-border/70 bg-background/80 px-4 py-3">
						...
						<Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
							...
							<CardContent className="grid gap-4">
								...
								<div className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm text-muted-foreground">
									<span>Profile changes are saved for the current inventory account.</span>
					...
					<div className="grid gap-6 lg:grid-cols-[320px_1fr]">
						<Card className="border-border/70 bg-background/75 shadow-sm backdrop-blur">
							<CardHeader>
								...
								<CardDescription>Keep your inventory account secure.</CardDescription>
							</CardHeader>
